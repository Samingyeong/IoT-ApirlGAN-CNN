...
def cal_pro_score(masks, amaps, max_step=200, expect_fpr=0.3):
    ...
    if not HAS_SKIMAGE:
        # scikit-image가 없으면 AU PRO 점수를 계산할 수 없음
        return 0.0
...
def test(args):
    ...
    # 체크포인트 로드 (CPU/CUDA 호환성 처리)
    # 체크포인트가 CUDA에서 저장되었을 수 있으므로 CPU로 매핑
    try:
        checkpoint = torch.load(args.checkpoint_path, map_location='cpu', weights_only=False)
    except TypeError:
        # weights_only 파라미터가 없는 구버전 PyTorch
        checkpoint = torch.load(args.checkpoint_path, map_location='cpu')
    except Exception as e:
        # 다른 오류 발생 시 CPU로 강제 로드
    ...
    with torch.amp.autocast(device_type='cuda', enabled=autocast_enabled), torch.no_grad():
    ...
    # CPU 환경에서 너무 오래 걸리지 않도록, 우선 앞 20장만 테스트
    max_samples = 20
    processed = 0
    ...
    for items in test_dataloader:
        ...
        with torch.no_grad(), torch.amp.autocast(device_type='cuda', enabled=autocast_enabled):
            ...
            for layer in range(len(patch_tokens)):
                ...
                anomaly_map = torch.softmax(anomaly_map, dim=1)[:, 1, :, :].to(torch.float32)
        ...
        if processed >= max_samples:
            logger.info(f"max_samples({max_samples})에 도달하여 조기 종료합니다.")
            break
